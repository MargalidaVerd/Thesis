...
Also, two new custom functions are defined to plot the ACF and PACF functions with a different format:
...
To extend the forecasting framework and provide a robust evaluation of model performance, we developed a set of utility functions that automate the rolling forecast process, visualize predictions at different horizons, and summarize forecast accuracy through key error metrics.

We begin by defining a model constructor called `fit_manual_model()`, which takes ARIMA orders $(p,d,q)$ as input and returns a function that fits a model with those parameters to any given time series. This functional approach allows for flexible reuse across different datasets and rolling windows without hardcoding model specifications.
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
rolling_forecast <- function(train_series, test_series, fitting_function, h_max = 5, horizons = c(1,3,5)) {
  
  test_len <- length(test_series)
  confidence_interval = 95
  outputs <- matrix(NA_real_, nrow = test_len, ncol = length(horizons),
  ...
  for (i in seq_len(test_len)) {
    ...
    forecast  <- forecast(model, h = h_max, level = confidence_interval)$mean
  ...
  make_single_plot <- function(h) {
    ggplot(filter(plot_df, Horizon == h | Model == "Actual"),
      ...
      scale_linetype_manual(values = c(Actual = "solid",
                                       Auto   = "dashed",
                                       Fixed  = "dashed")) +
      labs(title = paste("Horizon", substring(h, 2), "years"),
...
```{r}
print_error_summary_model <- function(actual,         
                                        preds_list,     
                                        horizons = c("h1", "h3", "h5"))
{
  y_orig <- actual                
  ...
  for (model_name in names(preds_list)) {
    pred_mat <- preds_list[[model_name]]
...
get_error_metrics <- function(country, p, d, q,
                              start_year,
                              horizons = c(1, 3, 5))
{
  train_raw <- country_splits[[country]]$train$number_deaths
  ...
  fit_fixed <- function(y) Arima(y, order = c(p, d, q))
  fit_auto  <- function(y) auto.arima(y, stepwise = FALSE, approximation = FALSE)

  h_max <- max(horizons)
  horizons_char <- paste0("h", horizons)

  pred_fixed_log <- rolling_forecast(train_log, test_log, fit_fixed,
                                     h_max = h_max, horizons = horizons)
  pred_auto_log  <- rolling_forecast(train_log, test_log, fit_auto,
                                     h_max = h_max, horizons = horizons)

  err_tbl <- print_error_summary_model(
    actual     = test_raw,                     
    preds_list = list(
      Manual = exp(pred_fixed_log),           
      Auto   = exp(pred_auto_log)
    ),
    horizons  = horizons_char
  )
...
Same function but for individual model are defined:

```{r}
rolling_predictions_single <- function(train_ts, test_ts, fit_fun,
                                       h_max = 5L, horizons = c(1L, 3L, 5L)) {
  rolling_forecast(train_series = train_ts,
                   test_series  = test_ts,
                   fitting_function = fit_fun,
                   h_max = h_max,
                   horizons = horizons)
}

plot_rolling_forecast_single <- function(test_ts, preds_log, horizon = 1L) {
  horizon <- as.integer(horizon)
  pred_raw <- exp(preds_log[, paste0("h", horizon)])
  ...
  ggplot(df, aes(x = Year)) +
    ...
    labs(title = paste("Rolling‑origin forecast – Horizon", horizon, "year(s)"),
         y = "Number of deaths", colour = "") +
    theme_minimal(base_size = 13)
}
```
...
```{r}
rolling_error_metrics_single <- function(actual_raw,
                                         preds_mat,
                                         horizons = c("h1", "h3", "h5")) {
  rows <- lapply(horizons, function(h) {
    ...
    data.frame(
      Horizon = h,
      ME   = mean(actual_raw[keep] - yhat[keep]),
      RMSE = rmse(actual_raw[keep], yhat[keep]),
      MAE  = mae(actual_raw[keep], yhat[keep]),
      MAPE = mape(actual_raw[keep], yhat[keep]) * 100,
      row.names = NULL
    )
  })
  bind_rows(rows)
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
acf(train_spain %>% diff())
pacf(train_spain %>% diff())
```
...
err_tbl <- rolling_error_metrics_single(
  actual_raw = exp(test_spain),
  preds_mat  = exp(preds_log)
)
print(err_tbl)
```
...
preds_log <- rolling_predictions_single(train_spain, test_spain, fitting_function(2,1,2))
err_tbl <- rolling_error_metrics_single(
  actual_raw = exp(test_spain),
  preds_mat  = exp(preds_log)
)
...
get_error_metrics("Spain",2,1,2,1951)
...
make_country_plots("Spain",2,1,2,1951)
```
...
```
The auto generated model is the ARIMA(1,0,0).
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
acf(train_switzerland)
pacf(train_switzerland)
```
...
The training series is stationary. The series ACF plot shows an exponential decay, and the PACF plot shows significant spikes at lag 1. We will study the AR(1) model.
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
fit_switzerland_100 <- Arima(train_switzerland, order = c(1,0,0))
fit_switzerland_100
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}

fit_switzerland_100 %>% forecast:: checkresiduals()
shapiro.test(residuals(fit_switzerland_100))
```

The residuals of the 1,0,0 model pass the Ljung-Box test but not the normality one.

```{r}
make_country_plots("Switzerland", 1,0,0, 1951)
get_error_metrics("Switzerland", 1,0,0, 1951)
```
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
acf(train_sweden %>% diff())
pacf(train_sweden %>% diff())
```
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
fit_switzerland_111 <- Arima(train_sweden, order = c(1,1,1))
fit_switzerland_111
```
...
```{r}
make_country_plots("Sweden", 1,1,1, 1951)
get_error_metrics("Sweden", 1,1,1, 1951)
```
### Bulgaria
...
preds_log <- rolling_predictions_single(train_bulgaria, test_bulgaria, fitting_function(1,1,0))
err_tbl <- rolling_error_metrics_single(
  actual_raw = exp(test_bulgaria),
  preds_mat  = exp(preds_log)
)
print(err_tbl)
```
At the **one-step horizon (h1)** the model already shows a noticeable negative bias (ME ≈ –9.2) and moderate error levels (RMSE ≈ 15.0, MAE ≈ 12.5, MAPE ≈ 13.6 %).

By **three steps (h3)**, the bias deepens sharply to about –29.3, and every error metric more than doubles: RMSE climbs to ≈ 32.5, MAE matches the bias at ≈ 29.3, and MAPE rises to ≈ 32.4 %. The fact that MAE ≈ |ME| signals that the bulk of the error is systematic under-prediction rather than random noise.

At the **five-step horizon (h5)**, the deterioration becomes dramatic. Bias reaches nearly –48.4, RMSE tops ≈ 51.4, MAE again mirrors the bias at ≈ 48.4, and MAPE soars to ≈ 59.2 %.

**Overall:** accuracy collapses as the forecast horizon lengthens. Errors scale roughly linearly with lead time, and the growing negative mean error shows the model increasingly under-predicts the target. By five steps ahead, nearly all error is attributable to this systematic bias, signalling the need for recalibration or alternative modelling if long-range forecasts are required.
...
```{r}
make_country_plots("Bulgaria", 1,1,0, 1964)
```


```{r}
get_error_metrics("Bulgaria", 1,1,0, 1964)
```
...
```{r}
preds_log <- rolling_predictions_single(train_estonia, test_estonia, fitting_function(0,1,0))
err_tbl <- rolling_error_metrics_single(
  actual_raw = exp(test_estonia),
  preds_mat  = exp(preds_log)
)
...
```{r}
make_country_plots("Estonia", 0,1,0, 1981)
get_error_metrics("Estonia", 0,1,0, 1981)
```
