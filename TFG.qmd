...
The manual model satisfy both assumptions on residuals.
...
```
The automatic one, too. We can check the accuracy metric on test data of both models:

```{r, warning=FALSE, message=FALSE, echo = FALSE}
results_arima_estonia<- run_country_forecast("Estonia", start_year = 1980, order_manual = c(1,1,0),
                          order_auto = c(0,1,0), horizons = c(1,3,5), use_xreg = FALSE)
