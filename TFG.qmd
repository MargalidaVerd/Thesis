...
```{r}
log_spain_data_train <- window(log_spain_data, end = c(2011))
log_spain_data_test <- window(log_spain_data, start = c(2012))

```

```{r}
fit_spain_train <- forecast::auto.arima(log_spain_data_train)
summary(fit_spain_train)

```


```{r}
horizon <- length(log_spain_data_test)
forecasted_spain_test <- forecast(fit_spain_train, h = horizon, bootstrap = TRUE)

```

```{r}

predicted_log <- forecasted_spain_test$mean
actual_log <- log_spain_data_test


accuracy(predicted_log, actual_log)


predicted_original <- exp(predicted_log)
actual_original <- exp(actual_log)

accuracy(predicted_original, actual_original)

```

```{r}
df_test_eval <- data.frame(
  year = 2012:2021,
...
ggplot(df_test_eval, aes(x = year)) +
