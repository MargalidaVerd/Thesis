...
fitting_function_arimax <- function(p, d, q, xreg_train) {
  function(y, xreg_y) {
    Arima(y, order = c(p, d, q), xreg = as.matrix(xreg_y))
  }
}

fit_auto_arimax <- function(y, xreg) {
  auto.arima(y, xreg = as.matrix(xreg))
}
```
...
rolling_forecast_arimax <- function(train_series, test_series,
  ...
  for (i in seq_len(test_len)) {
    ...
    if (length(rolling_series) != nrow(rolling_xreg)) {
      stop(sprintf("Length mismatch: y = %d, xreg = %d",
                   length(rolling_series), nrow(rolling_xreg)))
    }
    ...
    forecast_values <- forecast(model,
                                h = nrow(xreg_forecasting),
                                xreg = xreg_forecasting,
                                level = confidence_interval)$mean
    
    for (h in horizons) {
      idx <- i + h - 1
      if (idx <= test_len) {
        outputs[idx, paste0("h", h)] <- forecast_values[h]
      }
    }
...
make_country_plots_arimax <- function(country, p, d, q, start_year, horizons = c(1, 3, 5)) {
  ...
  make_horizon_plots_arimax(train_ts, test_ts, xreg_train = as.matrix(xreg_train),
                            xreg_test  = as.matrix(xreg_test), fit_fixed_fun, fit_auto_fun, horizons = horizons)
}
```

```{r, warning=FALSE, message=FALSE}
rolling_arimax_predictions_single <- function(train_series, test_series,
                                              xreg_train, xreg_test,
                                              fitting_function,
                                              max_steps = 5,
                                              horizons = c(1, 3, 5)) {
  rolling_forecast_arimax(train_series, test_series,
                          xreg_train, xreg_test,
                          fitting_function,
                          max_steps = max_steps,
                          horizons = horizons)
...
```{r, warning=FALSE, message=FALSE}
get_error_metrics_arimax <- function(country, p, d, q, start_year,
                                     xreg_train, xreg_test,
                                     horizons = c(1, 3, 5)) {
  ...
  fit_function_fixed <- fitting_function_arimax(p, d, q)
  fit_function_auto  <- function(y, xreg) auto.arima(y, xreg = xreg)
  
  max_steps <- max(horizons)
  horizons_char <- paste0("h", horizons)
  
  predicted_fixed_log <- rolling_arimax_predictions_single(train_log, test_log,
                                                           xreg_train, xreg_test,
                                                           fitting_function = fit_function_fixed,
                                                           max_steps = max_steps,
                                                           horizons = horizons)
  
  predicted_auto_log  <- rolling_arimax_predictions_single(train_log, test_log,
                                                           xreg_train, xreg_test,
                                                           fitting_function = fit_function_auto,
                                                           max_steps = max_steps,
                                                           horizons = horizons)
  
  error_summary_table <- print_error_summary_model(test_series = test_raw,
                                                   predictions_list = list(
                                                     Manual = exp(predicted_fixed_log),
                                                     Auto   = exp(predicted_auto_log)
                                                   ),
...
xreg_spain_train <- as.matrix(data.frame(
...
```
The ARIMAX proposed is ARIMA(2,0,0).
...
pred_arimax <- rolling_arimax_predictions_single(train_spain_short, test_spain_short,
                                                 xreg_spain_train, xreg_spain_test,
                                                 fit_auto_arimax)


plot_rolling_forecast_single(test_spain_short, pred_arimax, h =1 )
plot_rolling_forecast_single(test_spain_short, pred_arimax, h =3 )
plot_rolling_forecast_single(test_spain_short, pred_arimax, h =5 )

test_spain_short <- ts(log(countries_short[["Spain"]][["test"]][["number_deaths"]]), start = 2010, frequency = 1)

xreg_spain_test <- as.matrix(data.frame(
...
W can check the error measures in order to analyze the accuracy of the model.

```{r, warning=FALSE, message=FALSE, echo = FALSE}

error_summary <- rolling_error_metrics_single( test_series = exp(test_spain_short), predictions = exp(pred_arimax))
error_summary
...
```{r}
adf.test(train_spain_short %>% diff())
adf.test(ts(log(countries_short[["Spain"]][["train"]][["population"]]), start = 1960, frequency = 1) %>% diff())
adf.test(ts(log(countries_short[["Spain"]][["train"]][["gdp_capita"]]), start = 1960, frequency = 1) %>% diff())
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}
train_spain_short_diff <- train_spain_short %>% diff()

custom_acf_plot(train_spain_short_diff)
custom_pacf_plot(train_spain_short_diff)
```
...
custom_acf_plot(population_spain_diff)
custom_pacf_plot(population_spain_diff)
```
...
custom_acf_plot(gdp_capita_spain_diff)
custom_pacf_plot(gdp_capita_spain_diff)
```
...
get_error_metrics_arimax("Spain", 1,1,1, 1960, xreg_spain_train, xreg_spain_test)
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
make_country_plots_arimax("Spain", 1,1,1, 1960)
...
get_error_metrics_arimax("Netherlands", ,,, 1960, xreg_netherlands_train, xreg_netherlands_test)
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
make_country_plots_arimax("Netherlands", , , 1960)
```
