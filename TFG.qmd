...
rolling_forecast_arimax <- function(train_series, test_series,
                                    xreg_train, xreg_test,
                                    fitting_function, h_max = 5, horizons = c(1, 3, 5)) {
  ...
  for (i in seq_len(test_len)) {
    ...
    # Regresores para el pronóstico (h_max futuros)
    xreg_future <- xreg_test[i:min(i + h_max - 1, nrow(xreg_test)), , drop = FALSE]
    
    forecast_vals <- forecast(model, h = nrow(xreg_future), 
                              xreg = xreg_future,
                              level = confidence_interval)$mean
...
```{r}
make_horizon_plots_arimax <- function(train_series, test_series,
                                      xreg_train, xreg_test,
                                      fit_fixed_fun, fit_auto_fun,
                                      horizons = c(1,3,5)) {
  
  # Rolling forecasts
  pred_fixed <- rolling_forecast_arimax(
  ...
  pred_auto <- rolling_forecast_arimax(
    train_series, test_series,
    xreg_train, xreg_test,
    fitting_function = fit_auto_fun,
    h_max = max(horizons), horizons = horizons
  ...
  make_single_plot <- function(h) {
    ggplot(filter(plot_df, Horizon == h | Model == "Actual"),
      ...
      scale_linetype_manual(values = c(Actual = "solid",
                                       Auto   = "dashed",
                                       Fixed  = "dashed")) +
      labs(title = paste("Forecast horizon:", h),
           x = "Year", y = "Number of deaths",
           colour = "Model", linetype = "Model") +
      theme_minimal(base_size = 13)
  }
...
plots_spain <- make_horizon_plots_arimax(
  train_series = train_spain_short,
  test_series  = test_spain_short,
  xreg_train   = as.matrix(xreg_spain_train),
  xreg_test    = as.matrix(xreg_spain_test),
  fit_fixed_fun = fitting_function_arimax(1,1,1, xreg_spain_train), 
  fit_auto_fun  = fit_auto_arimax
)

# Visualizar uno de ellos
plots_spain$h1
plots_spain$h3
plots_spain$h5
