...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
fit_arima_complete_bulgaria_residuals <- Arima(complete_bulgaria_residuals, order = c(1, 0, 4))
...
### Modeling regressors -- prediction

```{r, warning=FALSE, message=FALSE, echo = FALSE}

plot_ts(ts(log(train_spain_df$population), start = 1951, frequency = 1)) + theme_minimal()
```
We try to fix an ARIMA model using auto.arima()

```{r, warning=FALSE, message=FALSE, echo = FALSE}

fit_pop_spain <- auto.arima(ts(log(train_spain_df$population), start = 1951, frequency = 1))
summary(fit_pop_spain)
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}
fit_pop_spain %>% forecast::checkresiduals()
shapiro.test(residuals(fit_pop_spain))

```

The residuals pass the Ljung-Box test, but not the normality test.

```{r, warning=FALSE, message=FALSE, echo = FALSE}

forecasted_pop_spain <- forecast(fit_pop_spain, level = c(95), h = get_length("Spain", "test", "population"), bootstrap = TRUE)

autoplot(forecasted_pop_spain)  + theme_minimal()
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}

test_pop_spain <- ts(country_splits[["Spain"]][["test"]][["population"]], start =2011, frequency = 1)

plot_pop_spain <- data.frame(
  Year = seq_along(test_pop_spain),
  Actual = test_pop_spain,
  Predicted = exp(forecasted_pop_spain$mean)
)


ggplot(plot_pop_spain, aes(x = Year)) +
  geom_line(aes(y = Actual, color = "Actual")) +
  geom_line(aes(y = Predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = "Actual vs Predicted population - ARIMA(4,1,0)",
       x = "Year (Test Set)",
       y = "Number of Deaths",
       color = "Legend") +
  theme_minimal()
...
Now we do the same for gdp per capita.


```{r, warning=FALSE, message=FALSE, echo = FALSE}

plot_ts(ts(log(spain_short_train_df$gdp_capita), start = 1960, frequency = 1)) + theme_minimal()
```
We try to fix an ARIMA model using auto.arima()

```{r, warning=FALSE, message=FALSE, echo = FALSE}
summary(lm(log(spain_short_train_df$gdp_capita) ~ spain_short_train_df$year))

lm(log(spain_short_train_df$gdp_capita) ~ spain_short_train_df$year) %>% forecast::checkresiduals()
auto.arima(residuals(lm(log(spain_short_train_df$gdp_capita) ~ spain_short_train_df$year)))

fit_gdp_spain <- auto.arima(ts(log(spain_short_train_df$gdp_capita), start = 1960, frequency = 1))
summary(fit_gdp_spain)
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}
fit_gdp_spain %>% forecast::checkresiduals()
shapiro.test(residuals(fit_gdp_spain))

```

The residuals pass the Ljung-Box test and the normality test.

```{r, warning=FALSE, message=FALSE, echo = FALSE}

forecasted_gdp_spain <- forecast(fit_gdp_spain, level = c(95), h = 12)

autoplot(forecasted_gdp_spain)  + theme_minimal()
```

```{r, warning=FALSE, message=FALSE, echo = FALSE}

test_gdp_spain <- ts(countries_short[["Spain"]][["test"]][["gdp_capita"]], start =2010, frequency = 1)

plot_gdp_spain <- data.frame(
  Year = seq_along(test_gdp_spain),
  Actual = test_gdp_spain,
  Predicted = exp(forecasted_gdp_spain$mean)
)


ggplot(plot_gdp_spain, aes(x = Year)) +
  geom_line(aes(y = Actual, color = "Actual")) +
  geom_line(aes(y = Predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = "Actual vs Predicted population - ARIMA(1,1,0)",
       x = "Year (Test Set)",
       y = "Number of Deaths",
       color = "Legend") +
  theme_minimal()
