...
ggplot(data, aes(x = year)) +
  ...
  labs(title = "Actual vs Fitted TB deaths in Spain",
       subtitle = "Zoomed-in segment of the TB mortality time series in Spain (1980–2021)",
       x = "Year", y = "Number of Deaths",
       color = "Legend") +
  ylim(0,1000) +
  xlim(1980,2021) +
...
ggplot(sweden_model_1_data, aes(x = year)) +
  ...
  labs(title = "Actual vs Fitted TB deaths in Sweden",
       subtitle = "Zoomed-in segment of the TB mortality time series in Sweden (1980–2021)",
       x = "Year", y = "Number of Deaths",
       color = "Legend") +
  ylim(0,300) +
  xlim(1980,2021) +
...
length(spain_data$number_deaths)
```
...
log_spain_data_train <- ts(log_spain_data[1:60], start = 1951, frequency = 1)
log_spain_data_test <- ts(log_spain_data[61:71], start = 2011, frequency = 1)

length(log_spain_data_train)
length(log_spain_data_test)
...
ggplot(test_log_spain_model_1, aes(x = year)) +
  ...
  labs(title = " Logarithmic TB deaths in Spain: Actual vs Predicted (2012–2021)",
...
ggplot(test_spain_model_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = "TB Deaths in Spain: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
length(netherlands_data$number_deaths)

log_netherlands_data <- ts(log(netherlands_data$number_deaths), start = c(1950), frequency = 1)

log_neth_data_train <- ts(log_netherlands_data[1:61], start = 1950, frequency = 1)
log_neth_data_test <- ts(log_netherlands_data[62:73], start = 2012, frequency = 1)

length(log_neth_data_train)
length(log_neth_data_test)
...
ggplot(test_log_neth_model_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = " Logarithmic TB deaths in Netherland: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
ggplot(test_original_neth_model_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = "TB Deaths in the Netherlands: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
length(sweden_data$number_deaths)

log_sweden_data <- ts(log(sweden_data$number_deaths), start = c(1951), frequency = 1)

log_sweden_data_train <- ts(log_sweden_data[1:61], start = 1951, frequency = 1)
log_sweden_data_test <- ts(log_sweden_data[62:72], start = 2012, frequency = 1)

length(log_sweden_data_train)
length(log_sweden_data_test)
...
ggplot(test_log_sweden_model_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = " Logarithmic TB deaths in Sweden: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
ggplot(test_original_sweden_model_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = "TB Deaths in Sweden: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
length(switzerland_data$number_deaths)
...
```{r}
log_switz_train <- ts(log_switzerland_data[1:60], start = 1951, frequency = 1)
log_switz_test <- ts(log_switzerland_data[61:71], start = 2011, frequency = 1)

length(log_switz_train)
length(log_switz_test)
```
...
length(romania_data$number_deaths)
...
```{r}
romania_train <- ts(romania_ts[1:50], start = 1959, frequency = 1)
romania_test <- ts(romania_ts[51:60], start = 2009, frequency = 1)

romania_test
length(romania_train)
length(romania_test)
```
...
ggplot(test_romania_1, aes(x = year)) +
  ...
  geom_line(aes(y = predicted, color = "Predicted"), linetype = "dashed") +
  scale_color_manual(values = c("Actual" = "#1f77b4", "Predicted" = "#ff7f0e")) +
  labs(title = " Logarithmic TB deaths in Romania: Actual vs Predicted (2012–2021)",
       x = "Year", y = "Number of Deaths", color = "Legend") +
  theme_minimal()
```
...
length(latvia_data$number_deaths)
...
```
```{r}
latvia_train <- ts(latvia_ts[1:31], start = 1980, frequency = 1)
latvia_test <- ts(latvia_ts[32:42], start = 2011, frequency = 1)
latvia_train
length(latvia_train)
length(latvia_test)
...
In this section we will study ARIMAX models; ARIMAX models are ARIMA models that include some exogenous variables in order to get better forecastings rather than basic ARIMA. In our study, we will consider two exogenous variables: Gross Domestic Product per capita and the country population. 

First, we visualize these series.

```{r}

ex_variables <- complete_data %>% 
  filter(country_name %in% c("Spain", "Switzerland", "Sweden", "Netherlands", "Latvia", "Romania")) %>% 
...
```
Now, the outlook is quite different; Switzerland leads the list, with a big rise from the 2000, exceeding the 75000 U.S dollars per capita. Following Switzerland, the Netherlands and Sweden have a very similar pattern. Next to them, Spain is the fourth country. It has a top in 2007, and then it shows a little decaying. Finally, Romania and Latvia close the list. 


```{r}
complete_data %>% 
  dplyr::select(country_name, gdp_capita) %>% 
  na.exclude() %>%  
  group_by(country_name) %>% 
  count() %>% 
  arrange(desc(n)) %>% 
  data.frame() %>% 
  filter(country_name %in% c("Romania", "Latvia"))
```

### Spain

We begin by studying Spain. First, we need to check if the exogenous variables and the study variable have the same length. Remember, we are forecasting on the logarithmic Spain training set.

```{r}
spain_data

length(spain_data$number_deaths)
length(spain_data$population)
length(na.omit(spain_data$gdp_capita))

```

Notice that the population regressor has the same length as the original series, but the gdp per capita time series no. 

We can shorten the number of deaths time series, in order to get the same length in both series.

#### ARIMAX with two regressors

```{r}
log_spain_short <- ts(log_spain_data[10:71], start  = 1960, frequency = 1)
log_spain_short_train <- ts(log_spain_short[0:51], start = 1960, frequency = 1)
log_spain_short_test <- ts(log_spain_short[51:62], start = 2010, frequency = 1)
log_gdp_spain <- ts(log(spain_data$gdp_capita)[10:71], start = 1960, frequency = 1)


log_gdp_spain_train <- ts(log_gdp_spain[0:51], start = 1960, frequency = 1)
log_gdp_spain_test <- ts(log_gdp_spain[51:62], start = 2010, frequency = 1)
```

```{r}

log_population_spain <- ts(log(spain_data$population)[10:71], start = 1960, frequency = 1)
log_population_spain_train <- ts(log_population_spain[0:51], start = 1960, frequency = 1)
log_population_spain_test <- ts(log_population_spain[51:62], start = 2010, frequency = 1)

xreg = data.frame(
  pop = as.numeric(log_population_spain_train),
  gdp = as.numeric(log_gdp_spain_train)
)

xreg_test = data.frame(
  pop = as.numeric(log_population_spain_test),
  gdp = as.numeric(log_gdp_spain_test)
)

fit_arimax_spain <- auto.arima(log_spain_short_train, xreg = as.matrix(xreg))
fit_arimax_spain


fit_arimax_spain_2 <- Arima(log_spain_data_train, order = c(0,2,1), xreg = log_population_spain_train)
fs_2 <- forecast(fit_arimax_spain_2, xreg = log_population_spain_test, h = horizon_spain)

fs <- forecast(fit_arimax_spain, xreg = as.matrix(xreg_test), h= horizon_spain)
autoplot(fs)
fs_exp <- exp(fs$mean)
```
