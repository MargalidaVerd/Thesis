...
log_population_spain <- ts(log(spain_data$population), start = 1951, frequency = 1)
log_population_spain_train <- ts(log_population_spain[0:60], start = 1951, frequency = 1)
log_population_spain_test <- ts(log_population_spain[61:71], start = 2011, frequency = 1)

fit_arimax_spain <- auto.arima(log_spain_data_train, xreg = log_population_spain_train)

fs <- forecast(fit_arimax_spain, xreg = log_population_spain_test, h= horizon_spain)
fs_exp <- exp(fs$mean)
```
