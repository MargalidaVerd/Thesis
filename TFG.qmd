...
train_spain <- ts(log(country_splits[["Spain"]]$train$number_deaths),
                  start = 1951, frequency = 1)

test_spain  <- ts(log(country_splits[["Spain"]]$test$number_deaths),
                  start = 2011, frequency = 1)

build_dlm_fun <- function(par){
  dlmModPoly(order = 2,
             dV  = exp(par[1]),        
             dW  = exp(par[2:3])) 
}
...
fit_mle  <- dlmMLE(train_spain, parm = init_par,
                   build = build_dlm_fun, hessian = TRUE)

dlm_spain <- build_dlm_fun(fit_mle$par)    
...
one_step_fc <- numeric(length(test_spain))

filter_obj <- dlmFilter(train_spain, dlm_spain)

for(i in seq_along(test_spain)){
  tmp <- dlmForecast(filter_obj, nAhead = 1)
  one_step_fc[i] <- tmp$f
  
  serie_actualizada <- c(train_spain, test_spain[1:i])
  
  cat(sprintf("Iter %02d  |  pred = %.4f  |  len = %d  |  último dato real = %.4f\n",
              i, one_step_fc[i], length(serie_actualizada),
              tail(serie_actualizada, 1)))
  filter_obj <- dlmFilter(serie_actualizada, dlm_spain)
}

one_step_fc_ts <- ts(one_step_fc,
                     start     = start(test_spain),
                     frequency = frequency(test_spain))
...
```{r}
plot_spain_log_rolling <- data.frame(
  Year             = seq_along(test_spain),
  Actual           = exp(test_spain),
  Predicted_rolling = exp(one_step_fc)
)

ggplot(plot_spain_log_rolling, aes(Year)) +
  geom_line(aes(y = Actual,            color = "Actual")) +
  geom_line(aes(y = Predicted_rolling, color = "Predicted (1-step)"),
            linetype = "dashed") +
  scale_color_manual(values = c("Actual"            = "#1f77b4",
                                "Predicted (1-step)" = "#ff7f0e")) +
  labs(title  = "Actual vs Predicted TB deaths – Spain (original scale)",
       x      = "Year (Test Set)",
       y      = "Number of deaths",
       color  = "Legend") +
  theme_minimal()
...
```{r}
error_summary_dlm_spain <- data.frame(
  ME = mean(plot_spain_log_rolling$Actual - plot_spain_log_rolling$Predicted_rolling),
  RMSE = rmse(plot_spain_log_rolling$Actual ,plot_spain_log_rolling$Predicted_rolling),
  MPE = mean((plot_spain_log_rolling$Actual - plot_spain_log_rolling$Predicted_rolling) / plot_spain_log_rolling$Actual) * 100,
  MAE = mae(plot_spain_log_rolling$Actual, plot_spain_log_rolling$Predicted_rolling),
  MAPE = mape(plot_spain_log_rolling$Actual ,plot_spain_log_rolling$Predicted_rolling) * 100
)

error_summary_dlm_spain
...
```{r, message=FALSE, warning=FALSE}
country_splits[["Estonia"]]$train
train_estonia <- ts(log(country_splits[["Estonia"]]$train$number_deaths),
                  start = 1981, frequency = 1)

test_spain  <- ts(log(country_splits[["Estonia"]]$test$number_deaths),
                  start = 2012, frequency = 1)


fit_mle_estonia  <- dlmMLE(train_estonia, parm = init_par,
                   build = build_dlm_fun, hessian = TRUE)

dlm_estonia <- build_dlm_fun(fit_mle_estonia$par)    
...
one_step_fc_estonia <- numeric(length(test_estonia))

filter_obj_estonia <- dlmFilter(train_estonia, dlm_estonia)

for(i in seq_along(test_estonia)){
  tmp <- dlmForecast(filter_obj_estonia, nAhead = 1)
  one_step_fc_estonia[i] <- tmp$f
  
  serie_actualizada_estonia <- c(train_estonia, test_estonia[1:i])
  
  cat(sprintf("Iter %02d  |  pred = %.4f  |  len = %d  |  último dato real = %.4f\n",
              i, one_step_fc_estonia[i], length(serie_actualizada_estonia),
              tail(serie_actualizada_estonia, 1)))
  filter_obj_estonia <- dlmFilter(serie_actualizada_estonia, dlm_estonia)
}

one_step_fc_estonia_ts <- ts(one_step_fc_estonia,
                     start     = start(test_estonia),
                     frequency = frequency(test_estonia))

one_step_fc_estonia_ts
```


```{r}
plot_estonia_log_rolling <- data.frame(
  Year             = seq_along(test_estonia),
  Actual           = exp(test_estonia),
  Predicted_rolling = exp(one_step_fc_estonia)
)

ggplot(plot_estonia_log_rolling, aes(Year)) +
  geom_line(aes(y = Actual,            color = "Actual")) +
  geom_line(aes(y = Predicted_rolling, color = "Predicted (1-step)"),
            linetype = "dashed") +
  scale_color_manual(values = c("Actual"            = "#1f77b4",
                                "Predicted (1-step)" = "#ff7f0e")) +
  labs(title  = "Actual vs Predicted TB deaths – Estonia (original scale)",
       x      = "Year (Test Set)",
       y      = "Number of deaths",
       color  = "Legend") +
  theme_minimal()
...
train_bulgaria <- ts(log(country_splits[["Bulgaria"]]$train$number_deaths),
                  start = 1964, frequency = 1)

test_spain  <- ts(log(country_splits[["Bulgaria"]]$test$number_deaths),
                  start = 2012, frequency = 1)


fit_mle_bulgaria  <- dlmMLE(train_bulgaria, parm = init_par,
                   build = build_dlm_fun, hessian = TRUE)

dlm_bulgaria <- build_dlm_fun(fit_mle_bulgaria$par)    
...
one_step_fc_bulgaria <- numeric(length(test_bulgaria))

filter_obj_bulgaria <- dlmFilter(train_bulgaria, dlm_bulgaria)

for(i in seq_along(test_bulgaria)){
  tmp <- dlmForecast(filter_obj_bulgaria, nAhead = 1)
  one_step_fc_bulgaria[i] <- tmp$f
  
  serie_actualizada_bulgaria <- c(train_bulgaria, test_bulgaria[1:i])
  
  cat(sprintf("Iter %02d  |  pred = %.4f  |  len = %d  |  último dato real = %.4f\n",
              i, one_step_fc_bulgaria[i], length(serie_actualizada_bulgaria),
              tail(serie_actualizada_bulgaria, 1)))
  filter_obj_bulgaria <- dlmFilter(serie_actualizada_bulgaria, dlm_bulgaria)
}

one_step_fc_bulgaria_ts <- ts(one_step_fc_bulgaria,
                     start     = start(test_bulgaria),
                     frequency = frequency(test_bulgaria))

one_step_fc_bulgaria_ts
```


```{r}
plot_bulgaria_log_rolling <- data.frame(
  Year             = seq_along(test_bulgaria),
  Actual           = exp(test_bulgaria),
  Predicted_rolling = exp(one_step_fc_bulgaria)
)

ggplot(plot_bulgaria_log_rolling, aes(Year)) +
  geom_line(aes(y = Actual,            color = "Actual")) +
  geom_line(aes(y = Predicted_rolling, color = "Predicted (1-step)"),
            linetype = "dashed") +
  scale_color_manual(values = c("Actual"            = "#1f77b4",
                                "Predicted (1-step)" = "#ff7f0e")) +
  labs(title  = "Actual vs Predicted TB deaths – Bulgaria (original scale)",
       x      = "Year (Test Set)",
       y      = "Number of deaths",
       color  = "Legend") +
  theme_minimal()
